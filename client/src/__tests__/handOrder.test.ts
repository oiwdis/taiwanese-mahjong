...
describe('reconcileHand', () => {
  it('keeps the existing order and appends a draw', () => {
    const previous = parseTiles('3m 1m 2m');
    const incoming = parseTiles('1m 2m 3m 9p');
    expect(reconcileHand(previous, incoming)).toEqual(parseTiles('3m 1m 2m 9p'));
  });
  ...
  it('starts from the incoming order when there is no previous hand', () => {
    expect(reconcileHand([], parseTiles('3m 1m 2m'))).toEqual(parseTiles('3m 1m 2m'));
  });
});
